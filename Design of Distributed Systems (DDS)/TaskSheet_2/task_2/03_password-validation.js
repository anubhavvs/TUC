...
passwordList.forEach((pwd) => {
  var minCount = parseInt(pwd.times.split("-")[0]);
  var maxCount = parseInt(pwd.times.split("-")[1]);
  var numOcc = pwd.password.split(pwd.letter).length - 1;
  if (numOcc >= minCount && numOcc <= maxCount) {
    console.log("VALID");
  } else {
    console.log("INVALID");
  }
